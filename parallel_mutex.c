...
int keys[NUM_KEYS];
//pthread_mutex_t lock;		//For part 1 and part 3


pthread_mutex_t lock1;
pthread_mutex_t lock2;
pthread_mutex_t lock3;		//For part4
...
insertion time on 40 threads BEFORE using mutex: 0.05672 s
retrieval time on 40 threads BEFORE using mutex: 3.1966 s

Certainly there is an overhead to adding a mutex

insertion time on 40 threads AFTER using mutex: 0.0284 s
retrieval time on 40 threads AFTER using mutex: 10.8199 s
*/
...
If updates are done in parallel, on different buckets, we can set locks on all those buckets where the updates are being done. Since, no two threads will go to write and rewrite in the same bucket, no keys would be lost.To implement this, we need to set multiple mutexes on the elements of the array of pointers (with type bucket_entry), which is used to store keys (in a linked list type structure) as per their respective buckets.
...
//pthread_mutex_unlock(&lock); removed for part 3 since not required. Locks are anyway not required for retrieve operations.
    }
...
//pthread_mutex_init(&lock, NULL);		//For part 1 and part 3

pthread_mutex_init(&lock1, NULL);
pthread_mutex_init(&lock2, NULL);
pthread_mutex_init(&lock3, NULL);		//For part 4
